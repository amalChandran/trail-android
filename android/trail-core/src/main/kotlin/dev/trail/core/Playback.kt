...
class TrailPlayer(effect: TrailEffect = TrailEffect(), autoPlay: Boolean = true) {
    ...
    private var playRequested = autoPlay
    ...
    fun play() { playRequested = true; if (effect.durationSeconds == 0.0) return; seekEndpoint = false; if (status == TrailPlaybackStatus.Finished) elapsedSeconds = 0.0; status = TrailPlaybackStatus.Playing }
    fun pause() { playRequested = false; if (status == TrailPlaybackStatus.Playing) status = TrailPlaybackStatus.Paused }
    fun replay() { playRequested = true; elapsedSeconds = 0.0; seekEndpoint = false; status = if (effect.durationSeconds > 0) TrailPlaybackStatus.Playing else TrailPlaybackStatus.Paused }
    fun seek(fraction: Double) { requireFraction(fraction, "seek"); playRequested = false; seekEndpoint = fraction == 1.0; elapsedSeconds = fraction * effect.durationSeconds; status = TrailPlaybackStatus.Paused }
    ...
    fun configure(effect: TrailEffect, reset: Boolean = false) {
        ...
        val normalized = if (oldDuration == 0.0) 0.0 else progress
        this.effect = effect
        if (reset) { seekEndpoint = false; playRequested = true }
        elapsedSeconds = if (reset) 0.0 else normalized * effect.durationSeconds
        status = when {
            effect.durationSeconds == 0.0 || !playRequested -> TrailPlaybackStatus.Paused
            !effect.repeats && elapsedSeconds >= effect.durationSeconds -> TrailPlaybackStatus.Finished
            else -> TrailPlaybackStatus.Playing
        }
    }
