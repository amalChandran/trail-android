...
class TrailView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) : View(context, attrs), Choreographer.FrameCallback {
    var path: TrailPath = TrailPath(emptyList()); set(value) {
        if (field === value) return
        field = value; player.replay(); stopClock(); updateRenderer(); schedule(); invalidate()
    }
    var effect: TrailEffect = TrailEffect(); set(value) {
        if (field === value) return
        field = value; player.configure(value); stopClock(); schedule(); invalidate()
    }
    ...
    val progress: Double get() = player.progress
    val isPlaying: Boolean get() = player.status == TrailPlaybackStatus.Playing
    private val motionObserver = object : ContentObserver(Handler(Looper.getMainLooper())) {
        override fun onChange(selfChange: Boolean) { stopClock(); schedule(); invalidate() }
    }
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) { stopClock(); updateRenderer(); schedule() }
    ...
    override fun onAttachedToWindow() {
        ...
        context.contentResolver.registerContentObserver(Settings.Global.getUriFor(Settings.Global.ANIMATOR_DURATION_SCALE), false, motionObserver)
        schedule()
    }
    override fun onDetachedFromWindow() {
        stopClock(); context.contentResolver.unregisterContentObserver(motionObserver); super.onDetachedFromWindow()
    }
    ...
    private fun schedule() {
        if (!scheduled && width > 0 && height > 0 && renderer.path.length > 0 && isAttachedToWindow && isShown && windowVisibility == VISIBLE && !motionReduced() && player.status == TrailPlaybackStatus.Playing) {
