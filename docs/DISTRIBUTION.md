...
The independent `trail-ios` repository puts `Package.swift` at its root, with no third-party package dependencies. Xcode: **File → Add Package Dependencies**, paste `https://github.com/amalChandran/trail-ios`, choose a tagged version, then select **TrailMapKit** for Apple Maps or **TrailUI** for Canvas. **TrailEffects** is optional; plugin authors need **TrailCore** only.

That remote is not created yet in this environment because GitHub CLI has no signed-in account. The prepared sibling `trail-ios` directory can be added as a **local package** immediately. iOS 17+, Swift tools 6.0; the package also compiles on macOS 14+.

After the repository and tag exist:
...
3. Create/push `trail-ios` and the Android feature branch; review changes before merging/tagging. Tag both platforms only after final consumer verification. Keep the shared fixture schema/version identical across repositories.
